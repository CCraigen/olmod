...
namespace GameMod
{
	...
	{
		// TEMPORARY - these should be settings variables but this will work for now
		...
		public static int LagCompMode = 0;
		//zpublic static bool LagCompNew = false;
		public static bool gaugesOn = false;
		// END TEMPORARY
		public static int InputBufferLength = 2; // this one is for release -- not determined yet
		...
		// grab a reference to the private m_player_state_history array so we don't have to keep Reflecting it
		...
		public static class MPServerOptimization_FixedUpdateProcessControlsInternal
		{
			...
			public static bool Prefix(PlayerShip __instance)
			{
				...
				if ((enabled && NetworkSim.m_resimulating) || (GameplayManager.IsDedicatedServer() && MPTweaks.ClientHasTweak(__instance.connectionToClient.connectionId, "cphysics"))) // supports client-side physics, also simplify resim
				{
					...
					//__instance.m_boost_heat = current.m_boost_heat;
					//__instance.m_boost_overheat_timer = current.m_boost_overheat_timer;
			...
			public static void Postfix(PlayerShip __instance)
			{
				...
				if (client != null && client.isConnected && GameplayManager.IsMultiplayerActive && !GameplayManager.IsDedicatedServer() && __instance.c_player.NeedToSendFixedUpdateMessages() && __instance.isLocalPlayer && !NetworkSim.m_resimulating)
				{
					//current.m_boost_heat = __instance.m_boost_heat;
					//current.m_boost_overheat_timer = __instance.m_boost_overheat_timer;
			...
			{
				// TODO - put a toggle here for joystick users so they can turn this off
				//return p.m_overdrive;
				//return false;
			...
			{
				if ((GameplayManager.IsDedicatedServer() || !RollFix) && mouseAimed) // only way we hit this as a server is if the client doesn't support the server optimizations, thus the roll fix -- TODO client-side check for server capability
				{
		...
		public class PlayerPhysicsMessage : PlayerInputMessage
		{
			...
			public override void Serialize(NetworkWriter writer)
			{
				if (enabled)
				{
					...
					foreach (PlayerEncodedPhysics p in m_inputs)
					{
						...
						//writer.Write(HalfHelper.Compress(p.m_boost_heat));
						//writer.Write(HalfHelper.Compress(p.m_boost_overheat_timer));
			...
			public override void Deserialize(NetworkReader reader)
			{
				...
				for (int i = 0; i < num; i++)
				{
					...
					//p.m_boost_heat = HalfHelper.Decompress(reader.ReadUInt16());
					//p.m_boost_overheat_timer = HalfHelper.Decompress(reader.ReadUInt16());
		...
		{
			//public float m_boost_heat;
			//public float m_boost_overheat_timer;
		...
		{
			public static PlayerEncodedInput SendPlayerPhysicsToServer(Player __instance)
			{
				NetworkClient client = Client.GetClient();
				...
				playerPhysicsMessage.m_inputs[ticks].m_encoded_bits = __instance.EncodePlayerButtonPresses();
				playerPhysicsMessage.m_inputs[ticks].m_encoded_bits |= __instance.GetEncodedInputFilter() << 16;
				playerPhysicsMessage.m_inputs[ticks].m_encoded_input = __instance.EncodeInput();
				...
				// We're also now -NOT- returning null anymore in the case of a send failure. There's a retry mechanic built-in to this with it sending multiple frames of input in each packet. The way it was originally had the possibility to drop inputs.
				/*if (!Client.GetClient().SendByChannel(62, playerPhysicsMessage, 2))
				{
					return null;
				}*/
		...
		*/
		...
		public static class MPServerOptimization_ApplyFixedUpdateInputMessage
		{
			...
			{
				if (NetworkSim.m_resimulating || (GameplayManager.IsDedicatedServer() && MPTweaks.ClientHasTweak(__instance.connectionToClient.connectionId, "cphysics")))
				{
		...
		/* Disabled for now while gauges are still active. Swap this in for the one in MPSkipClientResimulation when ready
				...
				for (int i = 0; i < num; i++)
				{
					foreach (Player player2 in Overload.NetworkManager.m_Players)
					{
						if (!(player2 == null) && !player2.m_spectator)
						{
							...
							player2.m_updated_state.m_tick = playerEncodedInputWithTick.m_tick; // Trying this here as well as in ProcessCachedControlsRemote() to see what happens -- I don't see a good reason why it *shouldn't* be here too
							...
							player2.ProcessRemotePlayerFiringControlsPost(); // and same here ^^
